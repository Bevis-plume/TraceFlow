"""
experiments/exp01_generation_baseline.py
=========================================
Experiment 01 — Generation Baseline

Config:  configs/defaults/exp01_baseline.yml
         (identity latent transform, no watermark)

Steps
-----
1. Train FlowTransformer on random data (smoke) / imagefolder (full).
2. Sample from the trained checkpoint.
3. Collect and write metrics.

Key metrics
-----------
  steps_completed, model_params_M, final_loss, device

CLI
---
    python -m experiments.exp01_generation_baseline --smoke
    python -m experiments.exp01_generation_baseline --smoke --dry-run
"""
...
EXP_ID = "exp01"
SMOKE_CONFIG = "configs/defaults/exp01_baseline.yml"
FULL_CONFIG = "configs/defaults/exp01_baseline.yml"
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    Args:
        args: Namespace with at minimum ``smoke``, ``dry_run``, ``output_dir``.

    Returns:
        Result dict with ``status``, ``exp_id``, ``metrics``, ``output_dir``.
    """
    ...
    print(f"\n[{EXP_ID}] Step 2/2: sample")
    run_command(sample_cmd, dry_run=dry_run)
    # Sample failures are non-fatal (report warns; training metrics are primary)
    ...
    if not dry_run:
        print(
            f"[{EXP_ID}] final_loss={metrics['final_loss']}  "
            f"model_params_M={metrics['model_params_M']}  "
